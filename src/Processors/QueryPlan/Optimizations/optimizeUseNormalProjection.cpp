...
std::optional<String> optimizeUseNormalProjections(Stack & stack, QueryPlan::Nodes & nodes)
{
    ...
    for (const auto * projection : normal_projections)
    {
        ...
        if (candidate.sum_marks >= ordinary_reading_marks && !(projection->name == context->getSettingsRef()[Setting::preferred_optimize_projection_name].value))
            continue;

        if (best_candidate == nullptr || candidate.sum_marks < best_candidate->sum_marks || projection->name == context->getSettingsRef()[Setting::preferred_optimize_projection_name].value)
